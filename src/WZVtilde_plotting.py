    ...
    for row in range(plot_row_dims):
        ...
        for col in range(plot_col_dims):
            if status == '0':
                ...
                if fixed ==None:
                    subplot_titles.append('Conditional on '+ plot_results[1][col].columns[0] +' and ' + plot_results[1][col].columns[1] + '<br> ||diff.||_2 = ' + str(twoNorm))
                else:
                    subplot_titles.append(fixed[col] + '<br> ||diff.||_2 = ' + str(twoNorm))
            else:
                if fixed ==None:
    ...
    for row in range(plot_row_dims):
        for col in range(plot_col_dims):
            if status == '0':
                twoNorm = float_formatter.format(np.linalg.norm(plot_results[1][col][var_name[row]] - plot_results[0][col][var_name[row]]))
                if fixed ==None:
                    subplot_titles.append('Conditional on '+ plot_results[1][col].columns[0] + '<br> ||diff.||_2 = ' + str(twoNorm))
                else:
                    subplot_titles.append(fixed[col] + '<br> ||diff.||_2 = ' + str(twoNorm))
            else:
                if fixed ==None:
