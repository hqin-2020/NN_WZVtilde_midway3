...
load_list = ['W_NN','Z_NN','V_NN','Vtilde_NN','logXiE_NN','logXiH_NN','kappa_NN','q_NN',\
             'sigmaK_NN', 'sigmaZ_NN','sigmaVtilde_NN','muK_NN','muZ_NN','muVtilde_NN','chi_NN',\
...
moment_list = ['W','Z','V','Vtilde','logXiE','logXiH','kappa','q','r','PiH_first_shock','PiH_second_shock','PiH_third_shock','PiE_first_shock','PiE_second_shock','PiE_third_shock','sigmaR_first_shock','sigmaR_second_shock','sigmaR_third_shock','dent']

W_NN, Z_NN, V_NN, Vtilde_NN, logXiE_NN, logXiH_NN, kappa_NN, q_NN,\
sigmaK_NN, sigmaZ_NN, sigmaVtilde_NN, muK_NN, muZ_NN, muVtilde_NN, chi_NN,\
sigmaW_NN, sigmaQ_NN, sigmaR_NN, deltaE_NN, deltaH_NN, PiH_NN, PiE_NN, betaE_NN, betaH_NN,\
muW_NN, muQ_NN, r_NN, dent_NN,\
...
NN_variables = pd.DataFrame([W_NN, Z_NN, V_NN, Vtilde_NN, logXiE_NN[:,0], logXiH_NN[:,0], kappa_NN[:,0], q_NN[:,0], r_NN[:,0], PiH_NN[:,0], PiH_NN[:,1], PiH_NN[:,2], PiE_NN[:,0], PiE_NN[:,1], PiE_NN[:,2], sigmaR_NN[:,0], sigmaR_NN[:,1], sigmaR_NN[:,2], dent_NN],\
                            index = moment_list).T
...
if domain_folder == 'WZV':
  ...
  NN_first_moment_conditional_WZV, NN_second_moment_conditional_WZV = calc_moment(NN_variables, ['W','Z','V'], moment_list)
  NN_first_moment_conditional_WZVtilde, NN_second_moment_conditional_WZVtilde = calc_moment(NN_variables, ['W','Z','Vtilde'], moment_list)
  NN_first_moment_conditional_WVVtilde, NN_second_moment_conditional_WVVtilde = calc_moment(NN_variables, ['W','V','Vtilde'], moment_list)
  NN_first_moment_conditional_ZVVtilde, NN_second_moment_conditional_ZVVtilde = calc_moment(NN_variables, ['Z','V','Vtilde'], moment_list)
  ...
  NN_first_moments_2d = [NN_first_moment_conditional_WZV, NN_first_moment_conditional_WZVtilde, NN_first_moment_conditional_WVVtilde, NN_first_moment_conditional_ZVVtilde]
  NN_second_moments_2d = [NN_second_moment_conditional_WZV, NN_second_moment_conditional_WZVtilde, NN_second_moment_conditional_WVVtilde, NN_second_moment_conditional_ZVVtilde]
...
if status != '3':
    ...
    if status == '0':
        ...
        MFR_variables = pd.DataFrame([W_MFR,Z_MFR,V_MFR,Vtilde_MFR,logXiE_MFR,logXiH_MFR,kappa_MFR,q_MFR[:,0],r_MFR[:,0],PiH_MFR[:,0],PiH_MFR[:,1],PiH_MFR[:,2],PiE_MFR[:,0],PiE_MFR[:,1],PiE_MFR[:,2],sigmaR_MFR[:,0],sigmaR_MFR[:,1],sigmaR_MFR[:,2],dent_MFR],\
                            index = moment_list).T
...
var_names = [['logXiE'], ['logXiH'], ['kappa'], ['q'], ['r'], ['PiH_first_shock', 'PiH_second_shock', 'PiH_third_shock'], ['PiE_first_shock', 'PiE_second_shock', 'PiE_third_shock'], ['sigmaR_first_shock', 'sigmaR_second_shock', 'sigmaR_third_shock']]
plot_contents = ['Experts Value Function',  'Households Value Function', 'Policy Function', 'Capital Price', 'Short Term Interest Rate', 'Households Risk Price', 'Experts Risk Price', 'Local Capital Return Volatility']

for i in range(len(plot_contents)):
    var_name = var_names[i]
    height = 1100 if len(var_name) > 1 else 600
