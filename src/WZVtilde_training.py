...
@tf.function 
def calc_var(valueFunctionLogH, valueFunctionLogE, constraintsFunctionKappa, W, Z, V, Vtilde, params, chi_range = 50, chi_position_tolerance = 1e-07, chi_value_tolerance = 1e-07):
    ...
    dW_logQtilde  = tf.gradients(logQtilde, W)[0];    dZ_logQtilde  = tf.gradients(logQtilde, Z)[0];    dVtilde_logQtilde = tf.gradients(logQtilde, Vtilde)[0]
    ...
    dW_Qtilde     = tf.gradients(Qtilde, W)[0];       dZ_Qtilde     = tf.gradients(Qtilde, Z)[0];       dVtilde_Qtilde    = tf.gradients(Qtilde, Vtilde)[0]
    ...
    dWdZ_Q        = tf.gradients(dW_Q, Z)[0];         dWdVtilde_Q       = tf.gradients(dW_Q, Vtilde)[0];         dZdVtilde_Q       = tf.gradients(dZ_Q, Vtilde)[0];
    dWdZ_logXiE   = tf.gradients(dW_logXiE, Z)[0];    dWdVtilde_logXiE  = tf.gradients(dW_logXiE, Vtilde)[0];    dZdVtilde_logXiE  = tf.gradients(dZ_logXiE, Vtilde)[0]; 
    dWdZ_logXiH   = tf.gradients(dW_logXiH, Z)[0];    dWdVtilde_logXiH  = tf.gradients(dW_logXiH, Vtilde)[0];    dZdVtilde_logXiH  = tf.gradients(dZ_logXiH, Vtilde)[0];
    ...
    if (params['chiUnderline'] >= 1) | (params['equityIss'] == 1):
      ...
      Dx                  = sigmaK + (sigmaZ*dZ_logQtilde + sigmaVtilde*dVtilde_logQtilde)                                  ###### eq. (70)
      ...
      def chi_2ndarg(chi_W):
        return (1 - W) * params['gamma_e']* Vtilde * tf.square(dW_logQtilde) * tf.math.pow(chi_W, 3) + (1 - W) * params['gamma_e']* Vtilde * dW_logQtilde * (W * dW_logQtilde - 2.0) * tf.square(chi_W) +\
                (( ( 1 - W) * params['gamma_e'] + W * params['gamma_h'] ) * DxNormSq + (1 - W) * params['gamma_e']* Vtilde * (1.0 - 2.0 * W * dW_logQtilde) +dW_logQtilde * DzetaX - DzetaOmega) * chi_W +\
                W* (1 - W) * (params['gamma_e'] - params['gamma_h']) * DxNormSq + W * (1 - W) * params['gamma_e'] * Vtilde - DzetaX
        
      chi                 = tfp.math.find_root_chandrupatla(chi_2ndarg, low=chi_Vtilde0-W-chi_range, high=chi_Vtilde0-W+chi_range, position_tolerance=chi_position_tolerance, value_tolerance=chi_value_tolerance, max_iterations=100, stopping_policy_fn=tf.reduce_all, validate_args=False,name='find_root_chandrupatla')[0]+W                                                                                                 
    ...
    variables = {'logXiE'    : logXiE,      'logXiH'    : logXiH,       'xiE'       : xiE,          'xiH'         : xiH,              'kappa'     : kappa,      'Q'         : Q,\
                 ...
                 'sigmaK'    : sigmaK,      'sigmaZ'    : sigmaZ,       'sigmaVtilde' : sigmaVtilde,\
                 'muK'       : muK,         'muZ'       : muZ,          'muVtilde'    : muVtilde,   'chi'       : chi,\
...
def HJB_loss_E(valueFunctionLogH, valueFunctionLogE, constraintsFunctionKappa, W, Z, V, Vtilde, params):
    ...
    sigmaZ = variables['sigmaZ'];   sigmaVtilde = variables['sigmaVtilde'];
...
def HJB_loss_H(valueFunctionLogH, valueFunctionLogE, constraintsFunctionKappa, W, Z, V, Vtilde, params):
    ...
    sigmaW = variables['sigmaW'];   sigmaZ = variables['sigmaZ'];       sigmaVtilde = variables['sigmaVtilde'];
...
## Training step BFGS
def training_step_BFGS(valueFunctionLogH, valueFunctionLogE, constraintsFunctionKappa, W, Z, V, Vtilde, params, targets, weight1, boundary1, weight2, boundary2):
  ...
  ## Train experts NN
  loss_fun = tf.keras.losses.MeanSquaredError()
  func_E = function_factory(valueFunctionLogE, HJB_loss_E, valueFunctionLogH, valueFunctionLogE, constraintsFunctionKappa, W, Z, V, Vtilde, params, loss_fun, targets, weight1=0, boundary1=2, weight2=0, boundary2=5)
  ...
  ## Train households NN
  loss_fun = tf.keras.losses.MeanSquaredError()
  func_H = function_factory(valueFunctionLogH, HJB_loss_H, valueFunctionLogH, valueFunctionLogE, constraintsFunctionKappa, W, Z, V, Vtilde, params, loss_fun, targets, weight1=0, boundary1=2, weight2=0, boundary2=5)
